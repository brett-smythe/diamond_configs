...
class InternsTwitterLimitsCollector(diamond.collector.Collector):
    """Collects stats for twitter limits API requests"""
    # pylint: disable=too-many-locals

    def collect(self):
        """Collect twitter API limits request metrics"""
        interns_service_logs_dir = '/var/log/interns-scheduler/'
        interns_base_log_name = 'scheduler.log'
        ...
        if oldest_dt.hour != utc_hour:
            ...
            number_of_limits_reqs += parse_log(older_log_name, oldest_dt)

        current_log_name = '{0}{1}'.format(
            interns_service_logs_dir, interns_base_log_name
        )
        number_of_limits_reqs += parse_log(current_log_name, oldest_dt)
        metric_name = 'twitter_api.limits_reqs_past_15'
        metric_value = number_of_limits_reqs
