...
class InternsTwitterTimeline(diamond.collector.Collector):
    ...
    def parse_log(self, log_path, oldest_datetime):
        ...
        with open(log_path) as inf:
            for line in inf:
                ...
                if log_msg == self.limits_req_line:
                    valid_requests += 1
    ...
    def collect(self):
        ...
        if oldest_dt.hour != utc_now.hour:
            ...
            number_of_limits_reqs += self.parse_log(older_log_name, oldest_dt)
        current_log_name = '{0}{1}'.format(
            self.interns_service_logs_dir, self.interns_base_log_name
        )
        number_of_limits_reqs += self.parse_log(current_log_name, oldest_dt)

        self.publish('twitter_limits_reqs_past 15', number_of_limits_reqs)
